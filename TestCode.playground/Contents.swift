...
extension Int {
    ...
    var manualSquareRoot: Int {
        ...
        while range.0 != range.1 {
            count += 1
            let testValue = Int(((range.0 + range.1) / 2))
            let pow = testValue ^^ 2
...
///subtract without using subtract
func --(base: Int, opperand: Int) -> Int {
    //flip sign and add one for negative value
    return base + (~opperand + 1)
}
